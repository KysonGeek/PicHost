...
- 自动生成缩略图（400×400）
- 图片画廊，支持大图预览、复制链接、删除
- HEIC（苹果照片）自动转为 JPEG 存储，并去除 EXIF 元数据（GPS/设备信息不外泄）
...
- **上传校验**：服务端按真实字节（Pillow）校验图片类型，不信任客户端声明的 `Content-Type`，非图片内容会被拒绝（HTTP 415）且不落盘。.glb 按 GLB 魔数与版本号校验。HEIC 上传会转码为 JPEG 并去除全部 EXIF 元数据。
